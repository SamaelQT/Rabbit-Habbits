<!--Detected and blocked with Kaspersky Lab-->
<!DOCTYPE html>
<html dir="ltr" lang="en" data-theme="&UiTheme&">
	<head>
		<title>Kaspersky Endpoint Security for Windows</title>
		<meta charset="UTF-8" />
		<meta content="IE=edge" http-equiv="X-UA-Compatible"/>
		<meta http-equiv="Pragma" content="no-cache" />
		<meta http-equiv="Cache-Control" content="no-cache" />
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<meta name="color-scheme" content="light dark" />
		<style type="text/css">
			:root {
    cursor: default;
    line-height: 1;
    overflow-wrap: break-word;
}
html, body, div, span, h1, h2, p, a, strong, b, ul, li, form, label, canvas, time, mark, button, input, u, i {
    margin:0;
    padding:0;
    border:0;
    vertical-align:baseline;
	box-sizing: border-box;
}
html, body {
	width:100%;
}
html {
	overflow: auto;
	height: auto;
}
a {
    background-color: transparent;
	outline-color: currentcolor;
	border-radius:2px;
	text-decoration: none;
	font-weight: 600;
}
b, strong {
    font-weight: bold;
}
button, [type="button"] {
    -webkit-appearance: button;
	overflow: visible;
}
button::-moz-focus-inner, [type="button"]::-moz-focus-inner {
    border-style: none;
    padding: 0;
}
button:-moz-focusring, [type="button"]:-moz-focusring {
    outline: 1px dotted ButtonText;
}
[hidden], .url-section-falsepositive-hidden {
    display: none;
}
button[disabled] {
    opacity: 0;
	cursor:default;
}
ol, ul {
    list-style-type: none;
}
iframe {
    border-style: none;
}
@font-face { 
  font-family: "KasperskySans";
  font-weight: normal;
  src: url(data:font/woff2;base64,
);
}

@font-face { 
  font-family: "KasperskySansBold";
  font-weight: 600;
  src: url(data:font/woff2;base64,);
}
body {
	font-family: 'KasperskySans','Segoe UI', Tahoma, sans-serif;
    font-weight: normal;
    font-style: normal;
	font-size: 100%;
	height:100%;
}
html[lang="IN"] {
	font-family: 'Nirmala UI','Segoe UI', Tahoma, sans-serif;
}
html[lang="CN"] {
	font-family: 'Microsoft YaHei UI','Segoe UI', Tahoma, sans-serif;
}
html[lang="HK"], html[lang="TW"] {
	font-family: 'Microsoft JhengHei UI','Segoe UI', Tahoma, sans-serif;
}
html[lang="JP"] {
	font-family: 'Yu Gothic UI','Segoe UI', Tahoma, sans-serif;
}
html[lang="KR"] {
	font-family: 'Malgun Gothic','Segoe UI', Tahoma, sans-serif;
}
html[lang="TH"] {
	font-family: 'Leelawadee UI','Segoe UI', Tahoma, sans-serif;
}

/* End Reset */
.background {
	padding: 56px 0 0 0;
}
.panel-wrap {
	margin: 50px auto 0;
	width:765px;
	background-position: 0 24px;
	background-repeat: no-repeat;
	position:relative;
}
.nojs {
	padding:24px;
	border-radius:12px;
	border-width:1px;
	border-style:solid;
	margin-bottom:24px;
	display: flex;
    align-items: center;
}
.nojs-text {margin:0 16px; display:inline-block;}
.panel {
	padding: 40px;
	border-radius:8px;
	font-size:1em;
}
.header {
	font-size:1.25em;
	line-height:1.2;
	margin-bottom:8px;
}
.picture {
	display:inline-block;
	vertical-align:top;
	width:48px;
	height:48px;
	background-repeat: no-repeat;
	background-position: center center;
}
.picture_nojs {
	width:24px;
	height:24px;
}
.content {
	display:inline-block;
	width:630px;
	padding-left:24px;
}
textarea {
	border-width:1px;
	border-radius:6px;
	border-style:solid;
	min-height:150px;
	width:500px;
	display:block;
}
p {
	margin-bottom:8px;
	line-height:1.25;
}
.flex-table {
	margin-bottom: 16px;
	display: grid;
	grid-template-columns: auto 1fr;
	grid-gap: 0 8px;
}
.flex-table-label {
	vertical-align: top;
	white-space: nowrap;
}
.flex-table-content {
	font-weight: 600;
	flex-grow:1;
}
.flex-table-content_trimmed {
	white-space: nowrap;
	overflow: hidden;
	text-overflow: ellipsis;
	line-height: 1.3;
	cursor:pointer;
	position:relative;
	padding: 0 16px 0 0;
}
.flex-table-content_trimmed::after {
	content: ' ';
	width:16px;
	height:16px;
	position:absolute;
	right:0;
}
html[dir='rtl'] .flex-table-content_trimmed {padding:0 0 0 16px;}
html[dir='rtl'] .flex-table-content_trimmed::after {
	left:0;
	right:auto;
}
.buttons {
	display:flex;
	padding: 16px 0;
}
.button { /* Outlines in focus-visible in future */
	padding:7px 16px;
	border-radius:5px;
	cursor:pointer;
	font:inherit;
	border-style: solid;
	border-width: 1px;
}
.button+.button {
	margin:0 16px;
}
.button b {
	position:relative;
}
.checkbox,.spoiler {
	margin-bottom:16px;
	display:table;
}
.checkbox-box {
	width:19px;
	height:19px;
	cursor:pointer;
	display:inline-block;
	vertical-align: middle;
}
.checkbox-label, .spoiler-label, .spoiler-label_less, .spoiler-label_more {
	user-select: none;
	-ms-user-select: none;
	cursor:pointer;
	vertical-align: middle;
}
.checkbox-label {
	padding: 0 8px;
	display:table-cell;
}
#spoiler:checked ~ .spoiler-content {
	display:block;
}
.spoiler-content, #spoiler:checked + label .spoiler-label_more, .spoiler-label_less {
	display:none;
}
#spoiler:checked + label .spoiler-label_less {
	display:inline;
}
.spoiler-label_arrow {
	width:16px;
	height:16px;
	margin:0 8px;
	vertical-align: middle;
}
#spoiler:checked + label .spoiler-label_arrow {
	transform: rotate(180deg);
}
.spoiler-content {
	padding-top:8px;
}
@keyframes rotation {
  from {
    transform: rotate(0deg);
  }
  to {
    transform: rotate(360deg);
  }
}
.spinner {
	width:24px;
	height:24px;
	animation: rotation linear 2s infinite;
}
#showTutorial {
	position:absolute;
	top:24px;
	right:0;
}
			.body-background,
html {
  background-color: #f5f7f9;
}
.background {
  background-image: url("data:image/svg+xml;base64,");
}
.picture_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.picture2_custom,
.pic-image_OtherReason_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.picture3_custom,
.panel-image-critical-cryptofraud_kes_custom,
.panel-image-critical_kes_custom {
  background-image: url("data:image/svg+xml;base64,");
  background-color: #FFECEA;
  border-radius: 24px;
}
.picture4_custom,
.picture_OtherReason_custom,
.picture_AdwareReason_custom,
.panel-image-warning-cryptofraud_kes_custom,
.panel-image-warning_kes_custom {
  background-image: url("data:image/svg+xml;base64,");
  background-color: #FFF1D6;
  border-radius: 24px;
}
.picture_success_custom {
  background-image: url("data:image/svg+xml;base64,");
  background-color: #DFF8F3;
  border-radius: 24px;
}
.panel-image-warning-cryptofraud_custom,
.panel-image-critical-cryptofraud_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.panel-image-warning_custom,
.panel-image-critical_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.picture7_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.picture8_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.picture8_custom,
.pic-image_AdwareReason_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.picture9_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.spinner {
  background-image: url("data:image/svg+xml;base64,");
}
.first-slide__image_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.second-slide__image_custom {
  background-image: url("data:image/svg+xml;base64,");
}
.third-slide__image_custom {
  background-image: url("data:image/svg+xml;base64,");
}
#brandColor {
  d: path("M 8,9 C 7,8 6.18,7 5.25,6.15 4.84,5.8 4.15,6 4,6.5 3.8,7.06 4.25,7.47 4.58,7.76 5.5,8.8 6.55,9.85 7.56,10.8 7.92,11.1 8.44,11 8.6,10.6 9.7,9.52 10.8,8.4 11.85,7.3 12.2,6.8 11.9,6 11.3,6 10.94,5.96 10.65,6.2 10.4,6.47 9.6,7.3 8.8,8.1 8,9 Z");
  fill: currentColor;
}
.nojs-text {
  color: #A5630C;
}
.nojs {
  border-color: #ef9928;
  background: #fdf3e6;
}
#tutorialOverlay {
  background: rgba(31, 50, 71, 0.27);
}
.body-background,
html {
  background-color: #f5f7f9;
}
.panel {
  background: #FFFFFF;
  color: rgba(42, 53, 65, 0.95);
  box-shadow: 0px 2px 6px rgba(24, 39, 75, 0.1);
}
.header {
  color: #0d0d15;
}
textarea {
  background: #FFFFFF;
  border-color: rgba(29, 54, 83, 0.17);
}
.button {
  color: #0d0d15;
  border-color: rgba(29, 54, 83, 0.17);
  background-color: #FFFFFF;
}
.button:hover {
  background-color: #e6e6e6;
}
.button:active {
  background-color: #cccccc;
}
.button_default {
  color: #ffffff;
  border-color: #1DA189;
  background-color: #1DA189;
}
.button_default:hover {
  background-color: #157664;
}
.button_default:active {
  background-color: #0d4b3f;
}
a,
.spoiler-label {
  color: #157664;
}
a:hover,
.spoiler-label:hover {
  color: #0d4b3f;
}
.copyable_url::after {
    background-image: url("data:image/svg+xml;base64,");
}
input[type="checkbox"] {
  accent-color: #1DA189;
}
button:focus,
input:focus,
a:focus {
  outline-color: #1DA189;
}
.tutorial-close {
  background-image: url("data:image/svg+xml;base64,");
}
#tutorial {
  background: #FFFFFF;
  color: #0d0d15;
}
.slider-dots-dot {
  background: rgba(29, 54, 83, 0.17);
}
.slider-dots-dot_active {
  background: rgba(42, 53, 65, 0.95);
}
@media (prefers-color-scheme: dark) {
  #tutorialOverlay {
    background: #00000080;
  }
  .body-background,
  html {
    background-color: #0d0d15;
  }
  .panel {
    background: #2e2c3f;
    color: rgba(209, 239, 245, 0.98);
    box-shadow: 0px 2px 6px rgba(24, 39, 75, 0.1);
  }
  .header {
    color: #f5f8fb;
  }
  textarea {
    background: #2e2c3f;
    border-color: rgba(208, 215, 251, 0.17);
  }
  .button {
    color: #f5f8fb;
    border-color: rgba(208, 215, 251, 0.17);
    background-color: rgba(212, 217, 242, 0.26);
  }
  .button:hover {
    background-color: rgba(173, 182, 230, 0.26);
  }
  .button:active {
    background-color: rgba(134, 148, 218, 0.26);
  }
  .button_default {
    color: #ffffff;
    border-color: #1DA189;
    background-color: #1DA189;
  }
  .button_default:hover {
    background-color: #157664;
  }
  .button_default:active {
    background-color: #0d4b3f;
  }
  a,
  .spoiler-label {
    color: #157664;
  }
  a:hover,
  .spoiler-label:hover {
    color: #0d4b3f;
  }
    .copyable_url::after {
        background-image: url("data:image/svg+xml;base64,");
    }
  input[type="checkbox"] {
    accent-color: #1DA189;
  }
  button:focus,
  input:focus,
  a:focus {
    outline-color: #1DA189;
  }
  .tutorial-close {
    background-image: url("data:image/svg+xml;base64,");
  }
  #tutorial {
    background: #2e2c3f;
    color: #f5f8fb;
  }
  .slider-dots-dot {
    background: rgba(208, 215, 251, 0.17);
  }
  .slider-dots-dot_active {
    background: rgba(209, 239, 245, 0.98);
  }
  .second-slide__image_custom {
    background-image: url("data:image/svg+xml;base64,");
  }
  .third-slide__image_custom {
    background-image: url("data:image/svg+xml;base64,");
  }
}

		</style>
	</head>
	<body class="body-background">
		<div class="background panel-wrap">		
			<div class="panel">
				<canvas testid="icon-deny" class="picture picture3_custom"></canvas>
				<div class="content">
					<h1 class="header" testid="deny-message">The requested web page cannot be provided.<br><br>Address: <a href="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js">https://cdn.jsdelivr.net/npm/chart.js...</a>.<br><br>The web page has been blocked by the CP-Blocked-Web-2025 rule.<br><br>Reason: the web resource belongs to the Undetermined content category(-ies) and the Undetermined data type category(-ies).<br><br>This web resource is prohibited at the company. If you consider the blocking to be mistaken or if you need to access this web resource, contact the administrator of the local corporate network at <a href="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js?2038BFDA-73A1-4D3C-86D1-DA727103C03B_kes_cup_C6FA3ED5_6D17_47D1_B6E2_F4B02CC905E0_">Request access</a>.<br>	</h1>
					<p testid="timestamp">Message generated: 4/10/2026 1:53:21 PM</p>
				</div>
			</div>
		</div>
	</body>
</html><!-- KL-CF-ResponseTemplate WebControl- -->