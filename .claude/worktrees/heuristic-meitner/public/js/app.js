...
async function toggleTask(id,itemEl){
  ...
  const pts = task.pointsAwarded || 0;
  if(task.completed){
...
async function renderStats(stats,s,e,globalStreak){
  ...
  for(let d=new Date(startD);d<=endD;d.setDate(d.getDate()+1)){
    const ds=tds(d),bd=stats.byDate[ds];
  ...
  state.habits.forEach(h=>{
    ...
    row.querySelectorAll('.hrm-cell:not(.hrm-future)').forEach(cell=>{
      cell.addEventListener('click',async()=>{
        ...
        if(log.done){
          const quote = COMPLETION_QUOTES[Math.floor(Math.random()*COMPLETION_QUOTES.length)];
          toast(`${h.emoji} +${Math.abs(pts)}⭐ ${quote}`);
          showPointsToast(pts);
          updatePointsUI((_shopData.points||0) + pts);
          launchConfetti('low');
        } else {
          toast(`${h.emoji} Bỏ chọn — ${pts}⭐`);
          showPointsToast(pts);
          updatePointsUI(Math.max(0, (_shopData.points||0) + pts));
        }
      });
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  // Start roaming pets on all pages
  setTimeout(() => initRoamingPets(), 1500);

  // Init drag-and-drop feed tray
  initFeedTray();
});
...
// ── 50 câu triết lí, ca dao tục ngữ động viên khi hoàn thành task ──
const COMPLETION_QUOTES = [
  ...
  "Siêng năng thì có, biếng nhác thì không.",
  ...
  "Bạn đang trên đường đúng rồi đó!",
  "Mỗi bước nhỏ đều đưa bạn gần hơn mục tiêu!",
  "Kỷ luật là cầu nối giữa mục tiêu và thành tựu.",
  "Thành công không đến từ những gì bạn làm thỉnh thoảng, mà từ những gì bạn làm kiên trì.",
  "Hôm nay khó khăn, ngày mai sẽ dễ hơn nếu bạn không bỏ cuộc.",
  "Giọt mồ hôi hôm nay là nụ cười ngày mai.",
  "Bạn không cần phải giỏi để bắt đầu, nhưng cần bắt đầu để giỏi.",
  "Chậm mà chắc, thắng cuộc đua dài.",
  "Mỗi ngày là một cơ hội mới để trở nên tốt hơn.",
  "Sự kiên trì chính là siêu năng lực thực sự.",
  "Đừng so sánh với người khác, hãy so với chính mình ngày hôm qua.",
  "Tiến bộ, không phải hoàn hảo!",
  "Bạn mạnh hơn bạn nghĩ rất nhiều!",
  "Thành Rome không được xây trong một ngày.",
  "Hành trình vạn dặm bắt đầu từ một bước chân.",
  "Không ai có thể quay về quá khứ để bắt đầu lại, nhưng ai cũng có thể bắt đầu từ hôm nay.",
  "Tương lai thuộc về những người tin vào giấc mơ của mình.",
  "Nỗ lực không bao giờ phản bội bạn.",
  "Dù chậm đến đâu, bạn vẫn nhanh hơn người nằm trên sofa.",
  "Bạn đã làm được! Tiếp tục phát huy nhé!",
  "Kết quả tốt đẹp đến từ những thói quen nhỏ mỗi ngày.",
  "Đừng chờ cảm hứng, hãy tạo ra nó bằng hành động!",
  "Hôm nay bạn đã tiến thêm một bước — tuyệt vời!",
  "Thói quen tốt là nền tảng của cuộc sống tốt đẹp.",
  "Bạn đang đầu tư cho chính mình — thật đáng giá!",
  "Khó khăn hôm nay là sức mạnh ngày mai.",
  "Ai gieo gió ấy gặt bão — ai gieo kiên trì gặt thành công!",
  "Tích tiểu thành đại!",
  "Giỏi lắm! Cứ thế này thì thành công không xa đâu!",
];
...
function createGoalCard(g){
  ...
  card.querySelector('.gc-today-tick')?.addEventListener('click', async()=>{
    ...
    if(!wasDone){
      const quote = COMPLETION_QUOTES[Math.floor(Math.random()*COMPLETION_QUOTES.length)];
      toast(`✅ +${Math.abs(pts)}⭐ ${quote}`);
      showPointsToast(pts);
      updatePointsUI((_shopData.points||0) + pts);
      launchConfetti('medium');
      checkAndAwardBadges();
    } else {
      toast(`↩️ Bỏ tick mục tiêu — ${pts}⭐`);
      showPointsToast(pts);
      updatePointsUI(Math.max(0, (_shopData.points||0) + pts));
    }
  });
...
function updateInventoryUI() {
  ...
  // Also update purchased items display in shop
  const pf = document.getElementById('pi-food');
  const pw = document.getElementById('pi-water');
  const pfe = document.getElementById('pi-fert');
  const pfr = document.getElementById('pi-freeze');
  if (pf) pf.textContent = _shopData.food || 0;
  if (pw) pw.textContent = _shopData.water || 0;
  if (pfe) pfe.textContent = _shopData.fertilizer || 0;
  if (pfr) pfr.textContent = _shopData.streakFreezes || 0;
  // Update feed tray too
  if (typeof updateFeedTrayUI === 'function') updateFeedTrayUI();
}

// ── Points toast (floating +X / -X) ──
let _ptsToastEl;
function showPointsToast(pts) {
  ...
  const isNeg = pts < 0;
  _ptsToastEl.textContent = isNeg ? `${pts} ⭐` : `+${pts} ⭐`;
  _ptsToastEl.classList.toggle('negative', isNeg);
...
async function loadMyPets() {
  try {
    ...
    pets.forEach(pet => {
      ...
      // Mood display
      const moodText = pet.mood === 'tired' ? '😩 Mệt mỏi' :
                       pet.mood === 'very_tired' ? '😵 Rất mệt' :
                       pet.mood === 'wilted' ? '🥀 Héo' :
                       pet.mood === 'very_wilted' ? '😰 Sắp chết khô' :
                       pet.mood === 'dead' ? '💀 Đã mất' :
                       pet.moodEmoji ? `${pet.moodEmoji} Khỏe mạnh` : '😊 Khỏe mạnh';

      // Death warning text
      const warningText = pet.warning && pet.alive
        ? (isAnimal(pet.type)
            ? '⚠️ Hãy cho ăn trước khi quá muộn!'
            : '⚠️ Hãy tưới nước kẻo cây chết!')
        : '';

      // Profile page: show stats only, no feeding buttons (use drag-drop on screen instead)
      card.innerHTML = `
        ${pet.warning ? `<div class="pet-warning-badge">${warningText}</div>` : ''}
        <div class="pet-emoji" style="font-size:${petSize}px">${pet.emoji}</div>
        <div class="pet-name">${esc(pet.name)}</div>
        <div class="pet-type-label">${pet.lifeStage || ''} · Lv.${pet.level}</div>
        <div class="pet-mood-display">${moodText}</div>
        ...
        <div class="pet-stats-row">
          <span>🥕 ${pet.timesFed || 0}</span>
          <span>💧 ${pet.timesWatered || 0}</span>
          <span>🌿 ${pet.timesFertilized || 0}</span>
        </div>
        ${!pet.alive ? `<div class="pet-dead-overlay"><div style="font-size:36px">😢</div><div class="pet-dead-text">${isAnimal(pet.type) ? 'Đã mất vì không được cho ăn quá 5 ngày' : 'Đã chết khô vì không được tưới nước quá 7 ngày'}</div></div>` : ''}
      `;
  ...
  try {
    ...
    if (res.newBadges && res.newBadges.length > 0) {
      for (const b of res.newBadges) {
        toast(`🏅 Thành tựu mới: ${b.emoji} ${b.name}! +20⭐`);
        launchConfetti('high');
      }
      // Update points from badge rewards
      if (res.badgePoints > 0) {
        showPointsToast(res.badgePoints);
        updatePointsUI(res.points);
      }
      await loadBadges();
    }
...
navigateTo = function(page) {
  ...
  if (page === 'profile' && !_profilePetsInited) {
...
// Update purchased items display in shop
function updatePurchasedItemsUI() {
  const f = document.getElementById('pi-food');
  const w = document.getElementById('pi-water');
  const fe = document.getElementById('pi-fert');
  const fr = document.getElementById('pi-freeze');
...
// ═══════════════════════════════════════════
// ROAMING PET SYSTEM — Natural behavior
// Pets walk, run, sit, and react to clicks
// ═══════════════════════════════════════════

let _roamingPets = [];
let _roamingInited = false;
let _cachedPetList = [];

// Pet states: 'walking', 'running', 'sitting', 'idle', 'reacting'
// 5 random interaction reactions when clicking a pet
const PET_INTERACTIONS = {
  animal: [
    { emoji: '❤️', text: 'Rất vui khi gặp bạn!', anim: 'pet-bounce' },
    { emoji: '😻', text: 'Meo meo~ Thích quá!', anim: 'pet-spin' },
    { emoji: '🎵', text: '♪ Nhún nhảy vui vẻ ♪', anim: 'pet-dance' },
    { emoji: '🐾', text: 'Chạy đi chơi nào!', anim: 'pet-bounce' },
    { emoji: '⭐', text: 'Toả sáng lấp lánh!', anim: 'pet-sparkle' },
  ],
  plant: [
    { emoji: '✨', text: 'Lấp lánh!', anim: 'pet-sparkle' },
    { emoji: '🌸', text: 'Nở hoa rực rỡ!', anim: 'pet-bloom' },
    { emoji: '💧', text: 'Khát nước rồi~', anim: 'pet-bounce' },
    { emoji: '🎵', text: '♪ Lắc lư theo gió ♪', anim: 'pet-dance' },
    { emoji: '🌟', text: 'Rực rỡ quá!', anim: 'pet-spin' },
  ]
};

async function initRoamingPets() {
  if (_roamingInited) return;
  _roamingInited = true;

  try {
    _cachedPetList = await apiShop.pets();
    const alivePets = _cachedPetList.filter(p => p.alive);
    if (!alivePets.length) return;

    const container = document.getElementById('roaming-pets-container');
    if (!container) return;

    // Show ALL alive pets
    alivePets.forEach((pet, i) => {
      createRoamingPet(pet, i, container);
    });

    loadMyPetsShop(_cachedPetList);
  } catch(e) { console.error('initRoamingPets:', e); }
}

// Pick a random mood icon from the pet's mood icons array
function getRandomMoodIcon(pet) {
  if (pet.moodIcons && pet.moodIcons.length) {
    return pet.moodIcons[Math.floor(Math.random() * pet.moodIcons.length)];
  }
  return pet.moodEmoji || '';
}

// Pet size based on level: level 1 = 28px, level 10 = 52px
function getPetSize(level) {
  return Math.round(28 + ((level - 1) / 9) * 24);
}

function createRoamingPet(pet, index, container) {
  const el = document.createElement('div');
  el.className = 'roaming-pet';
  el.dataset.petId = pet._id;

  const moodIcon = getRandomMoodIcon(pet);
  const petSize = getPetSize(pet.level);

  // No frame/border — clean floating pet with just emoji, name, mood
  el.innerHTML = `
    <div class="roam-pet-inner-clean">
      <div class="roam-pet-emoji" style="font-size:${petSize}px">${pet.emoji}</div>
      <div class="roam-pet-label">
        <span class="roam-pet-name">${esc(pet.name)}</span>
        <span class="roam-mood-icon">${moodIcon}</span>
      </div>
    </div>
  `;
  ...
  // Click: do random action, then run away slightly faster
  el.addEventListener('click', (e) => {
    e.stopPropagation();
    const rp = _roamingPets.find(r => r.el === el);
    if (rp && rp.state !== 'reacting') {
      triggerPetInteraction(pet, el, rp);
    }
  });

  container.appendChild(el);

  const rpObj = { el, pet, index, state: 'idle', timer: null, sitTimer: null, moodInterval: null };
  _roamingPets.push(rpObj);

  // Periodically change mood icon
  rpObj.moodInterval = setInterval(() => {
    const iconEl = el.querySelector('.roam-mood-icon');
    if (iconEl) iconEl.textContent = getRandomMoodIcon(pet);
  }, 8000 + Math.random() * 5000);

  // Start behavior loop after staggered delay
  setTimeout(() => petBehaviorLoop(rpObj), 1000 + index * 3000);
}

// ── BEHAVIOR LOOP: walk → sit → walk → wander → sit ... ──
// All movements are SLOW and gentle. Only slightly faster when clicked.
function petBehaviorLoop(rp) {
  if (!rp || !rp.el.parentNode) return;

  const rand = Math.random();
  if (rand < 0.45) {
    petWalkToSpot(rp);   // Walk slowly to a random visible spot
  } else if (rand < 0.70) {
    petEnterAndSit(rp);  // Enter from edge and find a spot to sit
  } else if (rand < 0.90) {
    petWander(rp);       // Small gentle wander nearby
  } else {
    petSitDown(rp);      // Just sit where you are
  }
}

// Flip only the emoji, not the name/label (fixes reversed text)
function facePetDirection(el, goingRight) {
  const emojiEl = el.querySelector('.roam-pet-emoji');
  if (emojiEl) emojiEl.style.transform = goingRight ? 'scaleX(1)' : 'scaleX(-1)';
}

// Walk SLOWLY to a random spot on screen, then sit
function petWalkToSpot(rp) {
  rp.state = 'walking';
  const el = rp.el;

  const targetX = 60 + Math.random() * (window.innerWidth - 180);
  const targetY = 100 + Math.random() * (window.innerHeight - 280);
  const curX = parseFloat(el.style.left) || -100;

  facePetDirection(el, targetX > curX);
  el.classList.add('roam-walking');
  el.classList.remove('roam-sitting','roam-running');

  const dist = Math.abs(targetX - curX) + Math.abs(targetY - parseFloat(el.style.top));
  const walkDuration = Math.max(6000, dist * 18); // Very slow: ~18ms per px

  el.style.transition = `left ${walkDuration}ms ease-in-out, top ${walkDuration}ms ease-in-out`;
  el.style.left = targetX + 'px';
  el.style.top = targetY + 'px';

  rp.timer = setTimeout(() => {
    el.classList.remove('roam-walking');
    petSitDown(rp);
  }, walkDuration + 200);
}

// Sit at current position for a long while
function petSitDown(rp) {
  rp.state = 'sitting';
  const el = rp.el;
  el.classList.add('roam-sitting');
  el.classList.remove('roam-walking','roam-running');

  let idleCount = 0;
  const idleInterval = setInterval(() => {
    idleCount++;
    if (idleCount % 3 === 0) {
      el.classList.add('pet-idle-blink');
      setTimeout(() => el.classList.remove('pet-idle-blink'), 600);
    }
  }, 3000);

  // Stay sitting 10-25 seconds
  const sitDuration = 10000 + Math.random() * 15000;
  rp.sitTimer = setTimeout(() => {
    clearInterval(idleInterval);
    el.classList.remove('roam-sitting');
    petBehaviorLoop(rp);
  }, sitDuration);
}

// Small gentle wander — move to a nearby spot slowly
function petWander(rp) {
  rp.state = 'walking';
  const el = rp.el;
  el.classList.add('roam-walking');
  el.classList.remove('roam-sitting','roam-running');

  const curX = parseFloat(el.style.left) || 200;
  const curY = parseFloat(el.style.top) || 200;
  const dx = (Math.random() - 0.5) * 200;
  const dy = (Math.random() - 0.5) * 100;
  const targetX = Math.max(20, Math.min(window.innerWidth - 100, curX + dx));
  const targetY = Math.max(80, Math.min(window.innerHeight - 200, curY + dy));

  facePetDirection(el, targetX > curX);

  const wanderDuration = 5000 + Math.random() * 4000;
  el.style.transition = `left ${wanderDuration}ms ease-in-out, top ${wanderDuration}ms ease-in-out`;
  el.style.left = targetX + 'px';
  el.style.top = targetY + 'px';
...
// Enter from screen edge slowly and find a spot to sit
function petEnterAndSit(rp) {
  rp.state = 'walking';
  const el = rp.el;
  el.classList.add('roam-walking');
  el.classList.remove('roam-sitting','roam-running');
  ...
  const targetX = 80 + Math.random() * (window.innerWidth - 200);
  const targetY = 100 + Math.random() * (window.innerHeight - 280);

  el.style.transition = 'none';
  el.style.left = startX + 'px';
  el.style.top = targetY + 'px';
  facePetDirection(el, fromLeft);

  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      const walkDuration = 8000 + Math.random() * 6000; // Very slow entrance
      el.style.transition = `left ${walkDuration}ms ease-in-out, top ${walkDuration}ms ease`;
      el.style.left = targetX + 'px';
...
// ── CLICK: React then move away (slightly faster than normal) ──
function triggerPetInteraction(pet, el, rp) {
  clearTimeout(rp.timer);
  clearTimeout(rp.sitTimer);
  rp.state = 'reacting';

  const isAnimal = ['rabbit','cat','dog'].includes(pet.type);
  const interactions = isAnimal ? PET_INTERACTIONS.animal : PET_INTERACTIONS.plant;
  const interaction = interactions[Math.floor(Math.random() * interactions.length)];

  el.classList.remove('roam-walking','roam-sitting','roam-running',
    'pet-bounce','pet-spin','pet-dance','pet-sleep','pet-sparkle','pet-bloom');
  el.classList.add(interaction.anim);

  // Show popup above pet
  const popup = document.getElementById('pet-interact-popup');
  if (popup) {
    document.getElementById('pip-emoji').textContent = interaction.emoji;
    document.getElementById('pip-text').textContent = interaction.text;
    const rect = el.getBoundingClientRect();
    popup.style.left = Math.min(window.innerWidth - 120, Math.max(20, rect.left + rect.width / 2)) + 'px';
    popup.style.top = Math.max(10, rect.top - 50) + 'px';
    popup.style.display = 'block';
    popup.classList.add('pip-show');
    setTimeout(() => {
      popup.classList.remove('pip-show');
      popup.style.display = 'none';
    }, 1500);
  }

  // After reaction, move away at slightly faster pace (but not run)
  setTimeout(() => {
    el.classList.remove(interaction.anim);
    const curX = parseFloat(el.style.left) || 0;
    const runDir = curX > window.innerWidth / 2 ? -1 : 1;
    const newX = curX + runDir * (150 + Math.random() * 200);
    const newY = 100 + Math.random() * (window.innerHeight - 280);
    const clampedX = Math.max(20, Math.min(window.innerWidth - 80, newX));

    facePetDirection(el, runDir > 0);
    el.classList.add('roam-walking');
    // Slightly faster than normal walk but not a run
    el.style.transition = 'left 2.5s ease-out, top 2.5s ease-out';
...
// Load pets in shop page grid
function loadMyPetsShop(pets) {
  const grid = document.getElementById('my-pets-shop-grid');
  const empty = document.getElementById('my-pets-shop-empty');
  if (!grid) return;
  grid.querySelectorAll('.pet-card-mini').forEach(c => c.remove());
  if (!pets || !pets.length) { if (empty) empty.style.display = ''; return; }
  if (empty) empty.style.display = 'none';

  pets.forEach(pet => {
    const card = document.createElement('div');
    card.className = 'pet-card-mini' + (!pet.alive ? ' pet-dead' : '') + (pet.warning ? ' pet-warning' : '');
    const ptsInLevel = pet.totalPoints % 50;
    const pctLevel = Math.min(100, Math.round((ptsInLevel / 50) * 100));
    card.innerHTML = `
      ${pet.warning ? '<div class="pet-warning-badge">⚠️</div>' : ''}
      <div class="pcm-emoji">${pet.emoji}</div>
      <div class="pcm-name">${esc(pet.name)}</div>
      <div class="pcm-info">Lv.${pet.level} · ${pet.lifeStage || ''} ${pet.moodEmoji || ''}</div>
      <div class="pcm-bar"><div class="pcm-bar-fill" style="width:${pet.level >= 10 ? 100 : pctLevel}%"></div></div>
      ${!pet.alive ? '<div class="pcm-dead">😢 Đã mất</div>' : ''}
    `;
    grid.appendChild(card);
  });
}
...
// Drag food/water/fertilizer onto roaming pets
// ═══════════════════════════════════════════

function updateFeedTrayUI() {
  const f = document.getElementById('fi-food');
  const w = document.getElementById('fi-water');
  const fe = document.getElementById('fi-fert');
  if (f) f.textContent = _shopData.food || 0;
  if (w) w.textContent = _shopData.water || 0;
  if (fe) fe.textContent = _shopData.fertilizer || 0;
}

function initFeedTray() {
  const toggle = document.getElementById('feed-tray-toggle');
  const items = document.getElementById('feed-tray-items');
  ...
  // Drag-and-drop for each feed item
  let dragGhost = null;
  let dragAction = null;
  const emojiMap = { food: '🥕', water: '💧', fertilizer: '🌿' };

  document.querySelectorAll('.feed-item').forEach(item => {
    // Use pointer events for touch + mouse support
    item.addEventListener('pointerdown', (e) => {
      e.preventDefault();
      dragAction = item.dataset.action;

      // Check if item available
      if ((_shopData[dragAction] || 0) < 1) {
        toast(`❌ Hết ${dragAction === 'food' ? 'thức ăn' : dragAction === 'water' ? 'nước' : 'phân bón'}!`);
        return;
      }

      // Create ghost element
      dragGhost = document.createElement('div');
      dragGhost.className = 'feed-item-ghost';
      dragGhost.textContent = emojiMap[dragAction];
      dragGhost.style.left = e.clientX - 18 + 'px';
      dragGhost.style.top = e.clientY - 18 + 'px';
      document.body.appendChild(dragGhost);

      // Highlight nearby pets
      _roamingPets.forEach(rp => rp.el.classList.add('feed-highlight'));
    ...
    item.addEventListener('pointerup', async (e) => {
      ...
      dragGhost.remove();
      dragGhost = null;
      _roamingPets.forEach(rp => rp.el.classList.remove('feed-highlight'));

      // Check if dropped on a pet
      const x = e.clientX, y = e.clientY;
      let targetPet = null;
      for (const rp of _roamingPets) {
        const rect = rp.el.getBoundingClientRect();
        // Generous hit area
        const expanded = { left: rect.left - 20, top: rect.top - 20, right: rect.right + 20, bottom: rect.bottom + 20 };
      ...
      if (targetPet && targetPet.pet.alive) {
        try {
          const res = await apiShop.care(targetPet.pet._id, dragAction);
          ...
          toast(`${actionNames[dragAction]} ${targetPet.pet.name} (+${res.pointsGain} pts)`);

          // Play happy animation on the pet
          targetPet.el.classList.add('pet-sparkle');
          setTimeout(() => targetPet.el.classList.remove('pet-sparkle'), 1200);
        } catch (err) {
          toast('❌ ' + (err.message || 'Có lỗi xảy ra!'));
        }
      } else if (targetPet && !targetPet.pet.alive) {
        toast('😢 Pet này đã mất rồi...');
      }
      dragAction = null;
    });
